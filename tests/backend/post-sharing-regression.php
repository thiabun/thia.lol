...
assert_true(str_contains($postsSource, '$bioY = 258'), 'profile share card should reserve a fixed bio region');
assert_true(str_contains($postsSource, '$statsY = 408'), 'profile share card should reserve stats below the bio region');
assert_true(str_contains($postsSource, '$canonicalY = 538'), 'profile share card should keep the canonical URL in a fixed bottom region');
assert_true(str_contains($postsSource, 'posts_share_card_wrapped_text($image, $fonts, 24, $left, $bioY'), 'profile bio should be drawn through the bounded text wrapper');
assert_true(str_contains($postsSource, 'profile_share_card_draw_stats($image, $fonts, $profile, $left, $statsY'), 'profile stats should be drawn from the reserved stats band');
