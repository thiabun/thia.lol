...
assert_true(str_contains($postsSource, "header('Content-Type: image/jpeg')"), 'share-card endpoint should serve cached JPEG social cards');
...
assert_true(str_contains($postsSource, 'SHARE_CARD_RENDER_SCALE = 2'), 'share-card cache should store 2x captures');
...
assert_true(str_contains($postsSource, 'SHARE_CARD_MAX_UPLOAD_BYTES = 33554432'), 'share-card cache upload should allow high-resolution browser-rendered images');
...
assert_true(str_contains($postsSource, "share_card_existing_cache_path('post'"), 'post share-card endpoint should check cached screenshots first');
assert_true(str_contains($postsSource, "share_card_existing_cache_path('profile'"), 'profile share-card endpoint should check cached screenshots first');
assert_true(str_contains($postsSource, 'share_card_cached_image_response'), 'share-card endpoints should serve cached screenshots directly');
...
assert_true(str_contains($postsSource, "'image/jpeg' => 'jpg'"), 'share-card cache upload should accept JPEG data');
assert_true(str_contains($postsSource, "'image/png' => 'png'"), 'share-card cache upload should accept PNG data');
...
assert_true(str_contains($postsSource, 'Share card must be a 2400x1260 PNG or JPEG.'), 'share-card cache upload should enforce the high-resolution output size');
assert_true(str_contains($postsSource, '/uploads/share-cards/'), 'share-card cache should store public deterministic assets');
...
assert_true(str_contains($shareCaptureSource, 'const pixelRatio = options.pixelRatio ?? SHARE_CARD_PIXEL_RATIO'), 'share-card capture should default to 2x pixel ratio');
...
assert_true(str_contains($postShareModalSource, 'postShareCardCacheUpload'), 'post share modal should publish generated social cards to the cache endpoint');
assert_true(str_contains($postShareModalSource, 'type: "image/jpeg"'), 'post share modal should publish compact JPEG social cache cards');
...
assert_true(str_contains($profileShareModalSource, 'profileShareCardCacheUpload'), 'profile share modal should publish generated social cards to the cache endpoint');
assert_true(str_contains($profileShareModalSource, 'type: "image/jpeg"'), 'profile share modal should publish compact JPEG social cache cards');
...
assert_true(str_contains($profileShareRendererSource, "\$imageUrl = profile_share_page_https_url(post_public_base_url() . \$imagePath . '?v=' . profile_share_page_card_version(\$profile))"), 'profile share renderer should use absolute HTTPS versioned share-card images');
...
assert_true(str_contains($profileShareRendererSource, 'profile_share_page_card_image_type'), 'profile share metadata should emit cached card MIME type');
