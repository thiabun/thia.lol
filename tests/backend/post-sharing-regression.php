...
assert_true(str_contains($postsSource, "SHARE_CARD_CACHE_RENDER_VERSION = 'mosaic-v2'"), 'share-card cache should include a render-version key so stale card designs are bypassed');
...
assert_true(str_contains($shareSceneSource, 'h-[56px]'), 'browser-rendered card lockup should be visibly larger');
...
assert_true(str_contains($shareRendererSource, "'mosaic-v2'"), 'post share metadata version should include the browser-rendered card version');
...
assert_true(str_contains($profileShareRendererSource, "'mosaic-v2'"), 'profile share metadata version should include the browser-rendered card version');
