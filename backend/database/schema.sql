...
CREATE TABLE IF NOT EXISTS messages (
  ...
  body TEXT NOT NULL,
  deleted_at DATETIME NULL,
  reaction_version BIGINT UNSIGNED NOT NULL DEFAULT 0,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS message_attachments (
  ...
  UNIQUE KEY message_attachments_message_position_unique (message_id, position),
  KEY message_attachments_message_idx (message_id),
  KEY message_attachments_post_idx (post_id),
  KEY message_attachments_room_idx (room_id),
  ...
  CONSTRAINT message_attachments_post_fk
    FOREIGN KEY (post_id) REFERENCES posts(id)
    ON DELETE SET NULL,
  CONSTRAINT message_attachments_room_fk
    FOREIGN KEY (room_id) REFERENCES rooms(id)
    ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
