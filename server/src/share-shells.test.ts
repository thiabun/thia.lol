...
describe("share shell service", () => {
  it("uses cached frontend screenshot cards for post OG images", async () => {
    ...
    await writeFile(path.join(cacheDirectory, "pc359fe2da759-screenshot-v10.jpg"), "jpeg");
    ...
    expect(metaContent(response.html, "og:image")).toContain("/uploads/share-cards/posts/pc359fe2da759-screenshot-v10.jpg");
  ...
  it("selects the v9 profile card even when the prior v8 artifact remains", async () => {
    ...
    await writeFile(path.join(cacheDirectory, "thia-screenshot-v10.jpg"), "current jpeg");
    ...
    expect(metaContent(response.html, "og:image")).toContain("/uploads/share-cards/profiles/thia-screenshot-v10.jpg");
    expect(metaContent(response.html, "og:image")).not.toContain("screenshot-v8");
  });
  ...
  it("changes the profile card cache buster for writes within the same second", async () => {
    const { service, uploadRoot } = await shareShellFixture();
    const cacheDirectory = path.join(uploadRoot, "share-cards", "profiles");
    const cachePath = path.join(cacheDirectory, "thia-screenshot-v10.jpg");
    await mkdir(cacheDirectory, { recursive: true });
