...
describe("share card service", () => {
  ...
  it("renders missing post share cards through the frontend screenshot renderer", async () => {
    ...
    expect(existsSync(path.join(uploadRoot, "share-cards", "posts", "pc359fe2da759-screenshot-v10.jpg"))).toBe(true);
  });
  ...
  it("renders missing profile share cards through the frontend screenshot renderer", async () => {
    ...
    expect(existsSync(path.join(uploadRoot, "share-cards", "profiles", "thia-screenshot-v10.jpg"))).toBe(true);
  });
  ...
  it("ignores the prior v8 profile card and writes the current screenshot artifact", async () => {
    ...
    expect(existsSync(path.join(cacheDirectory, "thia-screenshot-v10.jpg"))).toBe(true);
  });
  ...
  it("purges current and legacy profile cards when a profile is no longer public", async () => {
    ...
    expect(existsSync(path.join(cacheDirectory, "thia-screenshot-v10.jpg"))).toBe(false);
  });
  ...
  it("purges current and legacy post cards when a post is no longer public", async () => {
    ...
    expect(existsSync(path.join(cacheDirectory, "pc359fe2da759-screenshot-v10.jpg"))).toBe(false);
  });
