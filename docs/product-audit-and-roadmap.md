...
- App routes currently include `/`, `/discover`, `/search`, `/rooms`, `/rooms/:slug`, `/chat`, `/notifications`, `/@/:handle`, `/admin`, `/login`, `/register`, `/terms`, `/privacy`, `/cookies`, `/community-guidelines`, `/copyright`, `/moderation`, `/legal`, and `/legal/contact`.
